...
export default function ManagePage() {
  ...
  const jobsBodyRef = useRef<HTMLTableSectionElement>(null);
  useFlipList(jobsBodyRef, project?.jobs.map((job) => job.id) ?? [], jobsDrag.dragIndex);
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="flex items-center justify-between gap-3 flex-wrap mb-4">
          <SectionHeader num="01" title={t("Jobs")} caption={t("Each construction scope. Click a row to manage its bids.")} />
          <button onClick={() => setShowCatManager((s) => !s)} className="btn inline-flex items-center gap-1.5 shrink-0">
            <Settings2 size={14} /> {t("Manage categories")}
          </button>
        </div>

        {showCatManager && (
          <div className="panel p-4 mb-4">
            <div className="label-mono mb-2">{t("Job Categories (shared across all projects)")}</div>
            <div className="flex flex-wrap gap-2 mb-3">
              {categories.map((c) => (
                <span key={c} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold" style={{ background: "var(--glass-2)", border: "1px solid var(--border)" }}>
                  {t(c)}
                  <button onClick={() => removeCategory(c)} className="text-ink-muted hover:text-red" aria-label={t("Remove {category}", { category: c })}>
                    <X size={12} />
                  </button>
                </span>
              ))}
            ...
            >
              <input value={newCat} onChange={(e) => setNewCat(e.target.value)} placeholder={t("Add a new category…")} className="field-input flex-1 max-w-xs" />
              <button type="submit" className="btn btn-blue inline-flex items-center gap-1.5">
                <Plus size={14} /> {t("Add")}
              </button>
            </form>
            <p className="text-[11px] text-ink-muted mt-2">{t("Adding or removing here syncs to every project instantly.")}</p>
          </div>
        )}
          ...
          <table className="border-collapse" style={{ tableLayout: "fixed", width: jobsTableWidth }}>
            ...
            <thead>
              <tr className="border-b-[1.5px] border-ink">
                <th />
                <ResizableTh label={t("Job")} col="job" startResize={jobResize} />
                <ResizableTh label={t("Start")} col="start" startResize={jobResize} />
                <ResizableTh label={<>{t("End")} <span className="normal-case opacity-70">{t("(optional)")}</span></>} col="end" startResize={jobResize} />
                <ResizableTh label={t("Status")} col="status" startResize={jobResize} />
                <ResizableTh label={t("Budget")} col="budget" startResize={jobResize} align="right" />
                <ResizableTh label={t("Approved Sub / Price")} col="approved" startResize={jobResize} />
                <th />
              </tr>
            </thead>
            <tbody ref={jobsBodyRef}>
              {project.jobs.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-6 text-center text-ink-muted text-sm">{t("No jobs yet. Add your first scope below.")}</td>
                </tr>
              ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2.5 text-right font-mono text-sm whitespace-nowrap">
                        {job.estimatedCost > 0 ? (
                          <span title={job.sourceItemId ? t("Auto-filled from remodel costs") : undefined}>
                            {fmtMoney(job.estimatedCost)}
                            {job.sourceItemId && <span className="label-mono ml-1 align-middle">{t("est")}</span>}
                          </span>
                        ) : (
                      ...
                      <td className="p-1.5">
                        <div className="flex items-center gap-1.5 justify-end">
                          ...
                          <button onClick={() => removeJob(job.id)} className="icon-btn flex items-center justify-center" aria-label={t("Remove job")}>
                            <X size={13} />
                          </button>
                        </div>
                      </td>
        ...
        <button onClick={addJob} className="btn btn-blue mt-3 inline-flex items-center gap-1.5">
          <Plus size={14} /> {t("Add job")}
        </button>
      </section>

      {/* 02 — Bids board (Kanban / Spreadsheet) */}
      <section>
        <SectionHeader num="02" title={t("Bids")} caption={t("Track every bid as a Kanban board (drag to change status) or an Excel-style spreadsheet across all jobs.")} />
        <JobsBidsBoard
          project={project}
      ...
      <section>
        <SectionHeader num="03" title={t("Bid Email Identity")} caption={t("Used to auto-fill bid-request emails sent from each job's bidder rows.")} />
        <div className="panel p-5 grid sm:grid-cols-3 gap-5">
          <TextField label={t("Your Company Name")} value={project.companyName} onChange={(v) => patch((p) => ({ ...p, companyName: v }))} placeholder="123 Construction" />
          <TextField label={t("Your Name (sender)")} value={project.senderName} onChange={(v) => patch((p) => ({ ...p, senderName: v }))} placeholder={t("Duy")} />
          <TextField label={t("Plans Link")} value={project.plansLink} onChange={(v) => patch((p) => ({ ...p, plansLink: v }))} placeholder="https://drive.google.com/…" hint={t("Included in the email so bidders can access the plans.")} />
        </div>
      </section>
