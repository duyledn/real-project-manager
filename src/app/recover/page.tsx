...
export default function RecoverPage() {
  ...
  return (
    <AuthFrame title="Reset password" subtitle="Use your 6-digit recovery PIN">
      {done ? (
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-green font-semibold">
            <Check size={18} /> Password updated.
          </div>
          <Link href="/login" className="btn btn-blue w-full justify-center">Back to sign in</Link>
        </div>
      ) : (
        <>
          <form onSubmit={submit} className="space-y-3.5">
            <Field label="Username" value={username} onChange={setUsername} autoFocus />
            <Field label="Recovery PIN" value={pin} onChange={(v) => setPin(v.replace(/\D/g, "").slice(0, 6))} placeholder="6 digits" />
          ...
          <div className="mt-4 text-[12.5px]">
            <Link href="/login" className="font-bold text-accent">Back to sign in</Link>
          </div>
        </>
      )}
    </AuthFrame>
